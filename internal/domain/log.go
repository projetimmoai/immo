...
// erreur rencontrée pendant le traitement d'une source de ticket (e-mail,
// message d'application, appel téléphonique... cf. LogType*) —, avec assez
// de contexte pour investiguer : la source, la copropriete et/ou la
// personne concernés, quand elles sont connues.
type Log struct {
	ID             int64
	CreatedAt      time.Time
	LogTypeID      int64 // FK -> log_type.id, NOT NULL : à fixer explicitement à l'insertion
	Message        *string
	TicketSourceID *int64 // FK -> ticket_source.id, si l'évènement est lié au traitement d'une source de ticket
